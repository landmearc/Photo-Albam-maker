...
def build_album_direct(
    ...
    for page_idx, page_rows in enumerate(pages):
        ...
        for img_path, name_no_ext, shoot_date_str, category, note in page_rows:
            ...
            _set_cell_border(c0, border_size_pt)
            ...
            _set_cell_border(c1, border_size_pt)
...
class App(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.stake_type_var = tk.StringVar(value="")
        self.note_batch_var = tk.StringVar(value="")
        self.batch_apply_category_var = tk.BooleanVar(value=True)
        self.batch_apply_stake_var    = tk.BooleanVar(value=False)
        self.batch_apply_note_var     = tk.BooleanVar(value=False)
        self.point_edit_var = tk.StringVar()
        self.preview_mode_var = tk.StringVar(value="pdf")
        self.output_name_var = tk.StringVar(value="写真帳")
        self.border_size_var = tk.IntVar(value=4)
        self.album_preview_page_var = tk.IntVar(value=0)
        self._album_preview_imgs: list = []   # GC防止用
    ...
    def _build_ui(self):
        ...
        # =============================================
        # LEFT PANE  ①〜③-2 + 点名一覧 + 点名修正
        # =============================================
        left_container = ttk.Frame(main)
        main.add(left_container, weight=4)
        ...
        # --- ① 統合ドロップ ---
        ttk.Label(left, text="① ファイルをドロップ（写真 / PDF / SIMAを一括受付）").pack(anchor="w")
        drop_all = ttk.Label(left,
            text="📂  写真 / PDF / SIMAファイルをここにドロップ",
            relief="ridge", padding=10, anchor="center")
        drop_all.pack(fill="x", pady=(2, 4))
        drop_all.drop_target_register(DND_FILES)
        drop_all.dnd_bind("<<Drop>>", self.on_drop_all)
        ...
        ttk.Button(btns, text="② EXIFで並び替え", command=self.sort_photos_by_exif).pack(side="left", padx=8)
        ttk.Button(btns, text="リサイズ", command=self.do_resize).pack(side="left", padx=4)
        ttk.Button(btns, text="リネーム実行", command=self.do_rename_global_prefix).pack(side="left", padx=4)

        rename_opt = ttk.LabelFrame(left, text="リネーム設定", padding=4)
        rename_opt.pack(fill="x", pady=(0, 6))
        rowr1 = ttk.Frame(rename_opt)
        rowr1.pack(fill="x")
        ttk.Radiobutton(rowr1, text="点名付き",    variable=self.rename_mode_var, value="point").pack(side="left")
        ttk.Radiobutton(rowr1, text="連番のみ",    variable=self.rename_mode_var, value="serial").pack(side="left", padx=8)
        ttk.Radiobutton(rowr1, text="リネームしない", variable=self.rename_mode_var, value="none").pack(side="left", padx=8)
        rowr2 = ttk.Frame(rename_opt)
        rowr2.pack(fill="x", pady=(3, 0))
        ttk.Label(rowr2, text="接頭語").pack(side="left")
        ttk.Entry(rowr2, textvariable=self.serial_prefix_var, width=7).pack(side="left", padx=3)
        ttk.Label(rowr2, text="開始番号").pack(side="left", padx=(8, 0))
        ttk.Spinbox(rowr2, from_=0, to=99999, textvariable=self.serial_start_var, width=5).pack(side="left", padx=3)
        ttk.Label(rowr2, text="桁数").pack(side="left", padx=(8, 0))
        ttk.Spinbox(rowr2, from_=1, to=10, textvariable=self.serial_digits_var, width=3).pack(side="left", padx=3)
        ttk.Checkbutton(rowr2, text="全角数字", variable=self.serial_fullwidth_var).pack(side="left", padx=8)

        # --- 写真一覧 (10行) ---
        photo_list_lf = ttk.LabelFrame(left, text="写真一覧", padding=4)
        photo_list_lf.pack(fill="x", pady=(0, 6))

        tree_wrap = ttk.Frame(photo_list_lf)
        tree_wrap.pack(fill="x", expand=False)

        cols = ("No", "元ファイル", "点名", "区分", "杭種", "説明", "resize出力", "リネーム後")
        self.tree = ttk.Treeview(tree_wrap, columns=cols, show="headings", height=10)
        for c in cols:
            self.tree.heading(c, text=c)
        self.tree.column("No",       width=36, anchor="e", stretch=False)
        self.tree.column("元ファイル", width=130)
        self.tree.column("点名",      width=65, stretch=False)
        self.tree.column("区分",      width=55, stretch=False)
        self.tree.column("杭種",      width=95)
        self.tree.column("説明",      width=110)
        self.tree.column("resize出力", width=105)
        self.tree.column("リネーム後", width=105)
        ...
        tree_btn_row = ttk.Frame(photo_list_lf)
        tree_btn_row.pack(fill="x", pady=(3, 0))
        ttk.Button(tree_btn_row, text="▲ 上に", command=self.move_photo_up).pack(side="left", padx=2)
        ttk.Button(tree_btn_row, text="▼ 下に", command=self.move_photo_down).pack(side="left", padx=2)

        # --- ③-1 PDF ---
        ttk.Label(left, text="③-1 測量図PDF → 点名抽出").pack(anchor="w", pady=(4, 0))
        self.pdf_label = ttk.Label(left, text="PDF: 未選択", foreground="gray")
        self.pdf_label.pack(anchor="w", pady=1)
        rowpdf = ttk.Frame(left)
        rowpdf.pack(fill="x", pady=(0, 4))
        ...
        # --- ③-2 SIMA ---
        ttk.Label(left, text="③-2 SIMA(.sim/.sima) → 点名抽出＋表示").pack(anchor="w", pady=(4, 0))
        self.sim_label = ttk.Label(left, text="SIMA: 未選択", foreground="gray")
        self.sim_label.pack(anchor="w", pady=1)
        rowsim = ttk.Frame(left)
        rowsim.pack(fill="x", pady=(0, 4))
        ...
        # --- 点名一覧 (5行) ---
        ttk.Label(left, text="点名一覧（抽出結果）").pack(anchor="w", pady=(4, 0))
        pts_frame = ttk.Frame(left)
        pts_frame.pack(fill="x", pady=(2, 4))
        pts_y = ttk.Scrollbar(pts_frame, orient="vertical")
        pts_x = ttk.Scrollbar(pts_frame, orient="horizontal")
        self.points_list = tk.Listbox(pts_frame, height=5, selectmode="extended",
        ...
        # --- 点名の手動修正 ---
        edit = ttk.LabelFrame(left, text="点名の手動修正", padding=4)
        edit.pack(fill="x", pady=(0, 6))
        rowe = ttk.Frame(edit)
        rowe.pack(fill="x")
        ttk.Label(rowe, text="点名:").pack(side="left")
        ttk.Entry(rowe, textvariable=self.point_edit_var, width=16).pack(side="left", padx=4)
        ttk.Button(rowe, text="追加", command=self.point_add).pack(side="left")
        ttk.Button(rowe, text="更新", command=self.point_update).pack(side="left", padx=3)
        ttk.Button(rowe, text="削除", command=self.point_delete).pack(side="left", padx=3)
        ttk.Button(rowe, text="↑", width=3, command=self.point_move_up).pack(side="right")
        ttk.Button(rowe, text="↓", width=3, command=self.point_move_down).pack(side="right", padx=3)

        # =============================================
        # CENTER PANE  ④⑤⑥
        # =============================================
        center_container = ttk.Frame(main)
        main.add(center_container, weight=1)

        center = ttk.Frame(center_container, padding=6)
        center.pack(fill="both", expand=True)

        # --- ④ 点名割当 ---
        assign = ttk.LabelFrame(center, text="④ 点名割当 / 並び替え", padding=6)
        assign.pack(fill="x", pady=(0, 8))
        ttk.Label(assign, text="点名ダブルクリック → 選択写真へ一括割当").pack(anchor="w")
        ...
        # --- ⑤ 一括設定（チェックボックス付き）---
        catf = ttk.LabelFrame(center, text="⑤ 一括設定（チェックした項目を選択/全写真へ設定）", padding=6)
        catf.pack(fill="x", pady=(0, 8))

        # 区分（チェック付き）
        rowc1 = ttk.Frame(catf)
        rowc1.pack(fill="x", pady=(0, 4))
        ttk.Checkbutton(rowc1, text="区分:", variable=self.batch_apply_category_var, width=6).pack(side="left")
        cat_combo = ttk.Combobox(rowc1, textvariable=self.category_var,
                                  values=["新設", "復元", "既設"], width=7, state="readonly")
        cat_combo.pack(side="left", padx=4)
        ttk.Button(rowc1, text="→手入力へ", command=self.copy_category_choice_to_manual, width=8).pack(side="left", padx=4)
        ttk.Entry(rowc1, textvariable=self.category_manual_var, width=10).pack(side="left")

        # 杭種（チェック付き）
        rowc2 = ttk.Frame(catf)
        rowc2.pack(fill="x", pady=(0, 4))
        ttk.Checkbutton(rowc2, text="杭種:", variable=self.batch_apply_stake_var, width=6).pack(side="left")
        stake_combo = ttk.Combobox(rowc2, textvariable=self.stake_type_var,
                                    values=["コンクリート杭", "プラスチック杭", "金属標", "金属鋲", "計算点"],
                                    width=14)
        stake_combo.pack(side="left", padx=4)
        ttk.Label(rowc2, text="（手入力可）").pack(side="left")

        # 説明（チェック付き）
        rowc3 = ttk.Frame(catf)
        rowc3.pack(fill="x", pady=(0, 4))
        ttk.Checkbutton(rowc3, text="説明:", variable=self.batch_apply_note_var, width=6).pack(side="left")
        ttk.Entry(rowc3, textvariable=self.note_batch_var, width=40).pack(side="left", padx=4)

        # 適用ボタン
        rowc4 = ttk.Frame(catf)
        rowc4.pack(fill="x", pady=(2, 0))
        ttk.Button(rowc4, text="選択写真へ一括設定", command=self.apply_batch_to_selected).pack(side="left")
        ttk.Button(rowc4, text="全写真へ一括設定",   command=self.apply_batch_to_all).pack(side="left", padx=6)

        # --- ⑥ 写真帳 ---
        alb = ttk.LabelFrame(center, text="⑥ 写真帳（保存先＝写真フォルダ）", padding=6)
        ...
        row_border = ttk.Frame(alb)
        row_border.pack(fill="x", pady=(0, 4))
        ttk.Label(row_border, text="罫線の太さ:").pack(side="left")
        for label, val in [("細（0.5pt）", 1), ("中（1pt）", 2), ("標準（2pt）", 4)]:
            ttk.Radiobutton(row_border, text=label, variable=self.border_size_var, value=val).pack(side="left", padx=4)

        ttk.Button(alb, text="写真帳を生成（.docx）", command=self.generate_album).pack(anchor="w", pady=2)

        # --- 写真帳プレビュー ---
        prev_lf = ttk.LabelFrame(center, text="写真帳プレビュー", padding=4)
        prev_lf.pack(fill="both", expand=True, pady=(0, 4))

        prev_nav = ttk.Frame(prev_lf)
        prev_nav.pack(fill="x", pady=(0, 3))
        ttk.Button(prev_nav, text="◀", width=3,
                   command=self._album_prev_page).pack(side="left")
        ttk.Button(prev_nav, text="▶", width=3,
                   command=self._album_next_page).pack(side="left", padx=2)
        self.album_page_label = ttk.Label(prev_nav, text="page: -/-")
        self.album_page_label.pack(side="left", padx=6)
        ttk.Button(prev_nav, text="更新", width=5,
                   command=self.render_album_preview).pack(side="right")

        self.album_preview_canvas = tk.Canvas(prev_lf, bg="white",
                                              highlightthickness=1,
                                              highlightbackground="gray70")
        self.album_preview_canvas.pack(fill="both", expand=True)
        self.album_preview_canvas.bind("<Configure>",
                                       lambda e: self.after(50, self.render_album_preview))

        # =============================================
        # RIGHT PANE  プレビュー
        # =============================================
        right_wrap = ttk.Frame(main, padding=4)
        main.add(right_wrap, weight=5)

        switch_bar = ttk.Frame(right_wrap)
        switch_bar.pack(fill="x", pady=(0, 4))
        ttk.Label(switch_bar, text="プレビュー表示:").pack(side="left")
        ttk.Radiobutton(switch_bar, text="PDF",  variable=self.preview_mode_var, value="pdf",
                        command=self.update_preview_mode).pack(side="left", padx=6)
        ttk.Radiobutton(switch_bar, text="SIMA", variable=self.preview_mode_var, value="sim",
                        command=self.update_preview_mode).pack(side="left")

        self.right_pane = ttk.Panedwindow(right_wrap, orient="vertical")
        self.right_pane.pack(fill="both", expand=True)

        # ---------- PDF/SIMA プレビュー (weight=6) ----------
        preview_host = ttk.Frame(self.right_pane)
        self.right_pane.add(preview_host, weight=6)

        self.pdf_preview_frame = ttk.LabelFrame(preview_host, text="PDFプレビュー", padding=4)
        ...
        ttk.Button(nav, text="－", width=4, command=self.pdf_zoom_out).pack(side="left", padx=(8, 2))
        ttk.Button(nav, text="＋", width=4, command=self.pdf_zoom_in).pack(side="left", padx=2)
        ttk.Button(nav, text="ズームリセット", command=self.pdf_zoom_reset).pack(side="left", padx=6)
        ...
        self.pdf_zoom_label.pack(side="left", padx=6)
        ...
        self.sim_preview_frame = ttk.LabelFrame(preview_host, text="SIMAプレビュー（背景なし）", padding=4)

        simnav = ttk.Frame(self.sim_preview_frame)
        simnav.pack(fill="x")
        ttk.Button(simnav, text="フィット",   command=self.sim_fit_view).pack(side="left")
        ttk.Button(simnav, text="ズーム100%", command=self.sim_zoom_100).pack(side="left", padx=4)
        ttk.Label(simnav, text="クリック半径(px):").pack(side="left", padx=(10, 3))
        ttk.Spinbox(simnav, from_=2, to=80, textvariable=self.sim_pick_radius_var, width=4).pack(side="left")
        ttk.Label(simnav, text="ドラッグ閾値(px):").pack(side="left", padx=(10, 3))
        ttk.Spinbox(simnav, from_=1, to=50, textvariable=self.sim_drag_threshold_var, width=4).pack(side="left")
        self.sim_info_label = ttk.Label(simnav, text="SIMA: -")
        self.sim_info_label.pack(side="left", padx=8)
        ...
        # ---------- 写真プレビュー (weight=4) ----------
        photo_box = ttk.LabelFrame(self.right_pane, text="写真プレビュー（選択写真）", padding=4)
        self.right_pane.add(photo_box, weight=4)
    ...
    def apply_batch_to_selected(self):
        sel = self.tree.selection()
        if not sel:
            messagebox.showinfo("未選択", "写真一覧で設定する写真を選択してください。")
            return
        apply_cat   = self.batch_apply_category_var.get()
        apply_stake = self.batch_apply_stake_var.get()
        apply_note  = self.batch_apply_note_var.get()
        if not apply_cat and not apply_stake and not apply_note:
            messagebox.showinfo("未選択", "一括設定する項目（区分/杭種/説明）にチェックを入れてください。")
            return
        cat   = self._get_effective_category()
        stake = self.stake_type_var.get().strip()
        note  = self.note_batch_var.get()
        for iid in sel:
            ...
            if 0 <= idx < len(self.photos):
                ...
                if apply_stake:
                    self.photos[idx].stake_type = stake
                if apply_note:
                    self.photos[idx].note = note
        self.refresh_photo_table()
        items = []
        if apply_cat:   items.append(f"区分={cat}")
        if apply_stake: items.append(f"杭種={stake}")
        if apply_note:  items.append(f"説明={note}")
        self.logln(f"一括設定: {' / '.join(items)} を {len(sel)}件に設定")
    ...
    def apply_batch_to_all(self):
        ...
        for ph in self.photos:
            if apply_cat:   ph.category   = cat
            if apply_stake: ph.stake_type = stake
            if apply_note:  ph.note       = note
        self.refresh_photo_table()
        items = []
        if apply_cat:   items.append(f"区分={cat}")
        if apply_stake: items.append(f"杭種={stake}")
        if apply_note:  items.append(f"説明={note}")
        self.logln(f"一括設定: {' / '.join(items)} を全写真に設定")
    ...
    def on_drop_all(self, event):
        """統合ドロップ: 写真/PDF/SIMAを自動振り分け"""
        ...
        if simas:
            self.load_sim(simas[0])
        if not imgs and not pdfs and not simas:
            self.logln("対応ファイルが見つかりませんでした（写真/PDF/SIMA）。")
    ...
    def _on_tree_click(self, event):
        ...
        if col == "#4":
            ...
            self.after(10, lambda: self._show_stake_type_combobox(row, col))
    ...
    def _show_stake_type_combobox(self, row_iid, col):
        bbox = self.tree.bbox(row_iid, col)
        if not bbox:
            return
        x, y, width, height = bbox
        ...
        if idx < 0 or idx >= len(self.photos):
            return
        current_val = self.photos[idx].stake_type or ""

        var = tk.StringVar(value=current_val)
        cb = ttk.Combobox(self.tree, textvariable=var,
                          values=["コンクリート杭", "プラスチック杭", "金属標", "金属鋲", "計算点"],
                          width=12)
        cb.place(x=x, y=y, width=max(width, 110), height=height + 2)
        cb.focus_set()
        ...
        def commit(e=None):
            ...
            val = var.get().strip()
            if 0 <= idx < len(self.photos):
                self.photos[idx].stake_type = val
                self.refresh_photo_table()
                self.logln(f"杭種変更: No.{idx+1} → {val}")
            try:
                cb.destroy()
            except Exception:
                pass
    ...
    def refresh_photo_table(self):
        ...
        for i, ph in enumerate(self.photos, start=1):
            ...
            cat = ph.category or ""
            stake = ph.stake_type or ""
            note = ph.note or ""
            self.tree.insert("", "end", values=(i, src, pt, cat, stake, note, rz, rn))
        self.after_idle(self.render_album_preview)

    def move_photo_up(self):
        sel = self.tree.selection()
        if not sel:
            return
        indices = sorted([self.tree.index(iid) for iid in sel])
        if indices[0] == 0:
            return
        for idx in indices:
            self.photos[idx - 1], self.photos[idx] = self.photos[idx], self.photos[idx - 1]
        self.refresh_photo_table()
        # 選択を維持
        all_iids = self.tree.get_children()
        new_sel = [all_iids[i - 1] for i in indices if i - 1 >= 0]
    ...
    def move_photo_down(self):
        ...
        if not sel:
            return
        indices = sorted([self.tree.index(iid) for iid in sel], reverse=True)
        if indices[0] == len(self.photos) - 1:
            return
        for idx in indices:
            self.photos[idx], self.photos[idx + 1] = self.photos[idx + 1], self.photos[idx]
        self.refresh_photo_table()
        all_iids = self.tree.get_children()
        new_sel = [all_iids[i + 1] for i in reversed(indices) if i + 1 < len(all_iids)]
        self.tree.selection_set(new_sel)
    ...
    def do_rename_global_prefix(self):
        ...
        if mode == "none":
            self.logln("リネームをスキップしました（リネームしない設定）")
            return
    ...
    def _album_prev_page(self):
        p = self.album_preview_page_var.get()
        if p > 0:
            self.album_preview_page_var.set(p - 1)
            self.render_album_preview()

    def _album_next_page(self):
        total = max(1, math.ceil(len(self.photos) / self.PER_PAGE))
        p = self.album_preview_page_var.get()
        if p < total - 1:
            self.album_preview_page_var.set(p + 1)
            self.render_album_preview()

    def render_album_preview(self):
        cv = self.album_preview_canvas
        cv.delete("all")
        self._album_preview_imgs = []

        n_photos = len(self.photos)
        per_page = self.PER_PAGE
        total_pages = max(1, math.ceil(n_photos / per_page))
        page = max(0, min(self.album_preview_page_var.get(), total_pages - 1))
        self.album_preview_page_var.set(page)
        self.album_page_label.config(text=f"page: {page + 1} / {total_pages}")
        ...
        page_photos = self.photos[page * per_page: (page + 1) * per_page]
        if not page_photos:
            cv.create_text(cw // 2, ch // 2, text="（写真なし）",
                           anchor="center", fill="gray50")
            return
        ...
        for i, ph in enumerate(page_photos):
            ...
            # セル外枠
            cv.create_rectangle(0, y0, cw - 1, y1 - 1, outline="#555", width=1)
            # 写真セルと情報セルの境界線
            cv.create_line(photo_w, y0, photo_w, y1, fill="#555")
            ...
            if os.path.isfile(img_path):
                try:
                    ...
                    im = ImageOps.exif_transpose(im)
                    max_pw = photo_w - pad * 2
                    max_ph = (y1 - y0) - pad * 2
                    im.thumbnail((max_pw, max_ph), Image.LANCZOS)
                    tkimg = ImageTk.PhotoImage(im)
                    self._album_preview_imgs.append(tkimg)
                    cx = pad + (max_pw - im.width) // 2
                    cy = y0 + pad + (max_ph - im.height) // 2
                    cv.create_image(cx, cy, image=tkimg, anchor="nw")
                    thumb_drawn = True
                except Exception:
                    pass
            if not thumb_drawn:
                cv.create_rectangle(pad, y0 + pad, photo_w - pad, y1 - pad,
                                    outline="#aaa", fill="#eee")
                cv.create_text(photo_w // 2, (y0 + y1) // 2,
                               text="No Image", fill="#aaa", font=("Meiryo", 7))

            # ── テキスト情報 ──
            ty = y0 + 4
            lh = max(12, int((y1 - y0 - 8) / 5))
            font_s = ("Meiryo", 7)
            font_b = ("Meiryo", 7, "bold")

            # ファイル名（リネーム後 > resize後 > 元）
            name = os.path.splitext(
                os.path.basename(ph.renamed_path or ph.resized_path or ph.src_path)
            )[0]
            cv.create_text(text_x, ty, text=name, anchor="nw", font=font_b,
                           width=cw - text_x - 2)
            ty += lh

            shoot = get_shoot_date_jp(ph.src_path)
            if shoot:
                cv.create_text(text_x, ty, text=shoot, anchor="nw", font=font_s,
                               width=cw - text_x - 2)
                ty += lh

            if ph.category:
                cv.create_text(text_x, ty, text=f"区分: {ph.category}",
                               anchor="nw", font=font_s, width=cw - text_x - 2)
                ty += lh

            if ph.stake_type:
                cv.create_text(text_x, ty, text=f"杭種: {ph.stake_type}",
                               anchor="nw", font=font_s, width=cw - text_x - 2)
                ty += lh

            if ph.note:
                cv.create_text(text_x, ty, text=f"説明: {ph.note}",
                               anchor="nw", font=font_s, width=cw - text_x - 2)
    ...
    def generate_album(self):
        ...
        try:
            build_album_direct(out_docx, rows, photos_per_page=4, img_width_mm=80,
                               border_size_pt=self.border_size_var.get())
